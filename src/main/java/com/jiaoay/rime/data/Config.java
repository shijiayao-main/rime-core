...
import com.jiaoay.rime.ime.keyboard.Sound;
import com.jiaoay.rime.ime.symbol.TabManager;
...
public class Config {
    ...
    public List<String> getKeyboardNames() {
        ...
        if (names == null) return keyboards;
        for (Object s : names) {
  ...
  */
    ...
    private String getColorSchemeName() {
        ...
        if (color.containsKey("dark_scheme") || color.containsKey("light_scheme"))
            hasDarkLight = true;
        return scheme;
    }
            ...
            try {
                ...
                if (s.startsWith("0x")) {
                    if (s.length() == 3 || s.length() == 4)
                        ...
                        s = String.format("#%06x", Long.decode(s.substring(2)));
                    else if (s.length() == 9) s = "#0" + s.substring(2);
                }
